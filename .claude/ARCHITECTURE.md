...
- **The hook's output cap is load-bearing: 9KB, recency first.** The harness spills hook output above ~10KB to a file and shows a 2KB preview (every Jumbo session from 2026-08-26 until the 2026-09-09 rewrite saw only PROJECT.md's head; the old 16KB cap had also cut the sprint lines and carry-forward before they were emitted). Sections are ordered so the cap can only cut the Map. `.last-session.md` is a stack of `## <date time> · <thread>` notes; the hook shows the last 36h (min. the newest, ≤7 lines each). The Map extraction is exact-match `^## Map$`.
