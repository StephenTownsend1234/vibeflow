...
**Pack friction/delight (max one line, most sessions none):** if the vibeflow workflow *itself* clearly fought the user this session (a step that confused, a rule that got in the way) or clearly paid off at a specific moment, append one line to `~/.claude/vibeflow/friction.md`: `- YYYY-MM-DD <project> · friction|delight — <what happened>`. Log only moments the user would recognize if read back; this feeds pack fixes, not a diary.
...
On that confirmation: apply all writes in parallel, including `.claude/.last-session.md` — a carry-forward of **≤8 lines** (what shipped, what's unverified/undeployed, where iteration stalled). The SessionStart hook injects only the head of this file, so anything longer silently truncates — point to sprint files for detail. **Merge, don't overwrite, when chats run in parallel:** the file is *today's digest*, not this chat's — if the existing entry (read in step 1) is same-day work this session didn't do, compress it to a dated one-liner kept under your new block, still within the 8-line cap; sprint files hold each thread's detail. A prior-day entry is superseded entirely. It's what a future session or parallel chat reads first, and the only trace a no-sprint Freebuild session leaves. Delete `.claude/.session-snapshot.md` (now reconciled). Archive any fully-checked sprint (`sprints/archive/<sprint-NNN>-<slug>.md`, next sequential NNN), then make the checkpoint commit (`docs(wrap): <summary>`). **Your own session's code changes get committed by path alongside it — or explicitly deferred with a reason ("leaving X uncommitted until the visual pass"). Never left silently floating in a shared tree** for another chat to sweep into an unrelated commit. Offer a push with it — a checkpoint that only lives on this laptop isn't a backup — but push only on their yes.
