...
**Where the sprint runs** — one recommendation before building, grounded in reality: the orientation's worktree/sprint lines (or `git worktree list`) plus the one thing you can't see — ask "any other sessions running right now?" only when parallel work looks likely.
- Nothing else in flight and low-risk → main, in place. Touches a migration, a native module, or a live prompt/function → `sprint/<slug>` branch — one motion to abandon if verification fails.
- Another sprint is (or will be) running in parallel → **one worktree per sprint**: suggest starting that session with the worktree toggle on its `sprint/<slug>` branch (or `git worktree add ../<repo>-<slug> sprint/<slug>`). Two sessions must never share one folder — a checkout has one checked-out branch, whatever the branch pickers show. Reuse a worktree that already holds the sprint's branch; if this session is already inside one, work here.
- Worktrees isolate files, not intent: two sprints touching the same module will still merge painfully — sequence those instead. Each worktree needs its own install/env and builds/tests locally from its own folder; shared cloud targets (deploys, edge functions, TestFlight) are singletons — ship those from main after the merge.
