...
- **Cold with a sprint**: read the sprint file + orientation gaps in one parallel batch (trust the sprint's Approach and Ground truth — don't re-investigate what plan mode grounded). Multiple in-flight sprints and no target named: one AskUserQuestion to pick, then proceed. Brief in four lines (sprint, approach, N/M steps, next up), ask "anything changed since this was planned?", then go.
- **No sprint (Freebuild)**: ask what they want to work on if unstated, load the working context for that target on demand, and iterate. If the work grows into real scope — several coherent steps, decisions worth recording — offer to capture it to `.claude/sprints/<slug>.md` per [../start/references/sprint-md-shape.md](../start/references/sprint-md-shape.md) (read it then) so `/wrap` can track it; until then nothing is written to `.claude/`.
...
- **Don't game the task.** Implement the general solution that's actually correct — never one that hard-codes the expected result or only satisfies the immediate check. If the task itself is wrong or infeasible, say so.
...
- **Design is part of the step, not polish.** A step that lands UI ships its states (empty, loading, error) and follows the sprint's Design block / the project's `.claude/design.md`. Feedback that arrives as a feeling ("calmer", "more premium") gets translated via `../design/vocabulary.md` into observable changes, played back before coding.
- **Research before inventing.** On unfamiliar or known-thorny ground — including any `[unknowns]` step — search for the canonical approach and pitfalls before writing code; don't rediscover a solved problem through failed attempts.
...
Before checking off a step that lands a new surface (an endpoint, a schema change, a shared type), sweep its callers, consumers, migrations, and env vars — update them now rather than leaving them silently broken.
